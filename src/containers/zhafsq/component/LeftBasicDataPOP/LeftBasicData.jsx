...
class LeftBasicData extends  Component{
    ...
    render(){
        const subdistrictData = this.props.subdistrictData
        ...
        return(
            <div>
                ...
                <div className="population_bar_block">
                    {subdistrictData ? subdistrictData.map((item, key) => {
                        return(
                            <div key={key} className="bar_data clearfix">
                                <p>{item.name}</p>
                                <div className="population_bar">
                                    {subdistrictDataColor.map((value, index) => {
                                        var vv = value.value
                                        var width = item[vv] ? item[vv]/item.total*360 + 'px' : 0
                                        return(
                                            <div key={index} style={{width: width, background: value.color}} className="bar_item">{item[vv] ? item[vv] : 0}</div>
                                        )
                                    })}
